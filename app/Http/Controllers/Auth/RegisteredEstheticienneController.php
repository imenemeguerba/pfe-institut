...
class RegisteredEstheticienneController extends Controller
{
    ...
    public function store(EstheticienneRegisterRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('register.esthe.verify.otp')
            ->with('success', 'A verification code has been sent to ' . $data['email']);
    }
    ...
    public function verifyOtp(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            $admin?->notifications()->create([
                ...
                'data'    => json_encode([
                    'message' => "👩‍🎨 New expert registration request: {$user->fullName()}",
                ]),
                'read_at' => null,
            ]);
        } catch (\Exception $e) {}
        ...
        return redirect()->route('login')
            ->with('success', 'Email verified ✅ Your application has been submitted. You will be notified once validated by the administrator.');
    }
