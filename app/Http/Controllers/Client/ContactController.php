    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            $admin?->notifications()->create([
                ...
                'data'    => json_encode([
                    'message' => "📩 New message from  {$request->user()->fullName()} : {$request->sujet}",
                ]),
                'read_at' => null,
            ]);
        } catch (\Exception $e) {}

        return back()->with('success', 'Your message has been sent to the administration. We will get back to you shortly.');
    }
}
