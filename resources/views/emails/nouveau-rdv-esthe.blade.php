...
<div class="wrapper">
    ...
    <div class="body">
        <p class="greeting">Hello, <strong>{{ $rdv->estheticienne->prenom }}</strong> 👋</p>
        <p class="msg">A new appointment has been <strong style="color:#059669;">booked</strong> and is waiting for your confirmation.</p>

        <div class="card">
            <div class="card-row">
                <span class="card-label">👤 Client</span>
                <span class="card-value">{{ $rdv->client->fullName() }}</span>
            </div>
            <div class="card-row">
        ...
        <p style="font-size:12px; font-weight:700; color:#1a1a2e; text-transform:uppercase; letter-spacing:1px; margin-bottom:10px;">Services to perform</p>
        @foreach($rdv->services as $service)
            <div style="display:flex; justify-content:space-between; align-items:center; padding:9px 14px; background:#fdf9ff; border-radius:8px; margin-bottom:6px; font-size:13px;">
                <span style="color:#374151; font-weight:600;">{{ $service->nom }}</span>
                <span style="color:#9ca3af; font-size:11px;">{{ $service->duree }} min</span>
            </div>
        @endforeach

        @if($rdv->notes)
            <div class="warning" style="margin-top:14px;">
                📝 <strong>Client note:</strong> {{ $rdv->notes }}
            </div>
        @endif

        <div class="divider"></div>
        <p style="font-size:12px; color:#94a3b8; text-align:center;">
            Please log in to confirm or decline this appointment.<br>
            <strong style="color:#b480ff;">Glow Institute</strong>
        </p>
    </div>
    <div class="footer">
