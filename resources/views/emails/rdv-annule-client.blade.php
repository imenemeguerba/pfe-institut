...
<div class="wrapper">
    ...
    <div class="body">
        ...
        <p class="msg">We regret to inform you that your appointment scheduled for <strong style="color:#1a1a2e;">{{ $rdv->date_debut->isoFormat("dddd D MMMM YYYY") }}</strong> at <strong style="color:#1a1a2e;">{{ $rdv->date_debut->format("H:i") }}</strong> has been <strong style="color:#dc2626;">cancelled</strong>.</p>
        <div class="card red">
            <div class="card-row"><span class="card-label">📅 Date</span><span class="card-value">{{ $rdv->date_debut->format("d/m/Y") }}</span></div>
            <div class="card-row"><span class="card-label">🕐 Time</span><span class="card-value">{{ $rdv->date_debut->format("H:i") }} → {{ $rdv->date_fin->format("H:i") }}</span></div>
            <div class="card-row"><span class="card-label">💆 Expert</span><span class="card-value">{{ $rdv->estheticienne->fullName() }}</span></div>
            <div class="card-row"><span class="card-label">💄 Services</span><span class="card-value">{{ $rdv->services->pluck("nom")->join(", ") }}</span></div>
        </div>
        @if($rdv->motif_refus)
        <div class="warning">💬 <strong>Reason:</strong> {{ $rdv->motif_refus }}</div>
        @endif
        @if($annulePar === "client")
        <p class="msg" style="margin-top:16px;">Your cancellation has been processed. Feel free to book a new appointment at any time.</p>
        @else
        <p class="msg" style="margin-top:16px;">We apologize for the inconvenience. Feel free to book a new appointment at any time.</p>
        @endif
        <div class="divider"></div>
        <p style="font-size:12px;color:#94a3b8;text-align:center;">For any questions, feel free to contact us.<br><strong style="color:#b480ff;">Glow Institute</strong> — We look forward to seeing you soon. 🌸</p>
    </div>
    <div class="footer">
