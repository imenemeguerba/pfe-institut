    ...
    <form method="POST" action="{{ route('admin.produits.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="f-row-2">
                ...
                <div class="f-group" style="margin-bottom:0;">
                    ...
                    <input type="text" name="nom" value="{{ old('nom') }}" required autofocus class="f-input" placeholder="e.g. Rose Face Cream">
                    @error('nom')<p class="f-error">{{ $message }}</p>@enderror
                </div>
            </div>
            <div class="f-group">
                <label class="f-label">Description</label>
                <textarea name="description" class="f-input" placeholder="Describe the product...">{{ old('description') }}</textarea>
            </div>

            <div id="prixStockSection">
                <div class="f-row-3" style="margin-bottom:0;">
                    <div class="f-group" style="margin-bottom:0;">
                        <label class="f-label">Price (DA) *</label>
                        <input type="number" name="prix" id="prixInput" value="{{ old('prix') }}" min="0" step="100" class="f-input" placeholder="0">
                        @error('prix')<p class="f-error">{{ $message }}</p>@enderror
                    </div>
                    <div class="f-group" style="margin-bottom:0;">
                        <label class="f-label">Initial Stock *</label>
                        <input type="number" name="stock" id="stockInput" value="{{ old('stock', 0) }}" min="0" class="f-input" placeholder="0">
                        @error('stock')<p class="f-error">{{ $message }}</p>@enderror
                    </div>
                    <div class="f-group" style="margin-bottom:0;">
                        <label class="f-label">Alert Threshold *</label>
                        <input type="number" name="seuil_alerte" id="seuilNormal" value="{{ old('seuil_alerte', 5) }}" min="0" class="f-input" placeholder="5">
            ...
            <div id="variantesActives" style="display:none;">
                ...
                <div class="f-group" style="margin-bottom:0;">
                    <label class="f-label">Alert Threshold *</label>
                    <input type="number" name="seuil_alerte" id="seuilVariante" value="{{ old('seuil_alerte', 5) }}" min="0" class="f-input" style="max-width:200px;" placeholder="5">
        ...
        <div class="form-card">
            ...
            <div class="img-upload-area">
                <div class="img-preview-box" id="previewBox"><i class="fa-solid fa-box"></i></div>
        ...
        <div class="form-card">
            ...
            <p style="font-size:11px;color:#c4b5fd;margin-bottom:12px;">Add variants if the product comes in different sizes. Leave empty if one price only.</p>
            ...
            <div id="varList">
                <div class="var-row" style="display:grid;grid-template-columns:1fr 120px 100px 36px;gap:8px;margin-bottom:8px;">
                    ...
                    <input type="number" name="variantes[0][prix]"  placeholder="0" min="0" step="100" class="f-input var-prix" oninput="updateAuto()">
                    <input type="number" name="variantes[0][stock]" placeholder="0" min="0" class="f-input var-stock" oninput="updateAuto()">
        ...
        <div class="form-footer">
            ...
            <button type="submit" class="btn-submit"><i class="fa-solid fa-floppy-disk"></i> Create Product</button>
        </div>
    </form>
</div>
</div>
...
function addVar() {
    ...
    row.innerHTML =
        ...
        '<input type="number" name="variantes[' + varIdx + '][prix]"  placeholder="0" min="0" step="100" class="f-input var-prix" oninput="updateAuto()">' +
        '<input type="number" name="variantes[' + varIdx + '][stock]" placeholder="0" min="0" class="f-input var-stock" oninput="updateAuto()">' +
        '<button type="button" onclick="removeVar(this)" style="width:36px;height:36px;border-radius:50%;background:white;color:#ef4444;border:1.5px solid rgba(239,68,68,0.2);cursor:pointer;display:flex;align-items:center;justify-content:center;"><i class="fa-solid fa-xmark"></i></button>';
...
function removeVar(btn) {
    ...
    if (rows.length > 1) { btn.closest('.var-row').remove(); updateAuto(); }
}
...
function updateAuto() {
    ...
    prixInputs.forEach(function(i) { if (parseFloat(i.value) > 0) hasVariante = true; });

    var prixSection = document.getElementById('prixStockSection');
    var varMsg      = document.getElementById('variantesActives');
    var prixMain    = document.getElementById('prixInput');
    var stockMain   = document.getElementById('stockInput');
    var seuilNormal = document.getElementById('seuilNormal');
    var seuilVar    = document.getElementById('seuilVariante');

    if (hasVariante) {
        prixSection.style.display = 'none';
        varMsg.style.display = 'block';
        var total = 0;
        stockInputs.forEach(function(i) { total += parseInt(i.value) || 0; });
        if (stockMain) stockMain.value = total;
        var minPrix = Infinity;
        prixInputs.forEach(function(i) { if (parseFloat(i.value) > 0) minPrix = Math.min(minPrix, parseFloat(i.value)); });
        if (prixMain && minPrix !== Infinity) prixMain.value = minPrix;
        if (seuilNormal && seuilVar) { seuilVar.name = 'seuil_alerte'; seuilNormal.removeAttribute('name'); }
    } else {
        prixSection.style.display = '';
        varMsg.style.display = 'none';
        if (seuilNormal && seuilVar) { seuilNormal.name = 'seuil_alerte'; seuilVar.removeAttribute('name'); }
    }
}
