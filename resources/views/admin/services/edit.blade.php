            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <x-text-input id="nom" name="nom" type="text" class="mt-1 block w-full" :value="old('nom', $service->nom)" required />
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $service->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <x-text-input id="prix" name="prix" type="number" min="0" step="100" class="mt-1 block w-full" :value="old('prix', $service->prix)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('prix')" />
                        </div>
                        <div>
                            <x-input-label for="duree" :value="__('Durée (minutes) *')" />
                            <x-text-input id="duree" name="duree" type="number" min="5" max="480" step="5" class="mt-1 block w-full" :value="old('duree', $service->duree)" required />
                    ...
                    @if ($service->image)
                        <div class="mb-6">
                            ...
                            <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->nom }}" class="mt-1 w-32 h-32 object-cover rounded">
                        </div>
                    @endif

                    {{-- Nouvelle image --}}
                    <div class="mb-6">
                        <x-input-label for="image" :value="$service->image ? __('Remplacer l\'image (optionnel)') : __('Ajouter une image')" />
                    ...
                    <div class="mb-6">
                        ...
                        @if ($estheticiennes->isEmpty())
                            <p class="mt-2 text-sm text-gray-500 italic">
                                Aucune esthéticienne active pour l'instant.
                            </p>
                        @else
                    ...
                    <div class="mb-6">
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="actif" value="1" {{ old('actif', $service->actif) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
