...
<div class="fac-wrap">
    ...
    <div class="adm-hero">
        <div class="adm-hero-inner">
            <div>
                ...
                <div class="adm-hero-sub">All generated invoices — appointments and orders</div>
            </div>
    ...
    <div class="fac-tabs">
        <a href="{{ route('admin.factures.index', array_merge(request()->query(), ['type' => ''])) }}"
           class="fac-tab {{ $type === '' ? 'active' : '' }}">
            <i class="fa-solid fa-list"></i> All <span class="fac-tab-count">{{ $counts['toutes'] }}</span>
        </a>
        <a href="{{ route('admin.factures.index', array_merge(request()->query(), ['type' => 'rendez_vous'])) }}"
           class="fac-tab {{ $type === 'rendez_vous' ? 'active' : '' }}">
            <i class="fa-solid fa-spa"></i> Appointments <span class="fac-tab-count">{{ $counts['rdv'] }}</span>
        </a>
        <a href="{{ route('admin.factures.index', array_merge(request()->query(), ['type' => 'commande'])) }}"
           class="fac-tab {{ $type === 'commande' ? 'active' : '' }}">
            <i class="fa-solid fa-cart-shopping"></i> Orders <span class="fac-tab-count">{{ $counts['commandes'] }}</span>
        </a>
    </div>
    ...
    <div class="fac-table-card">
        @if($factures->isEmpty())
            <div class="fac-empty"><i class="fa-solid fa-file-invoice"></i><p>No invoices found.</p></div>
        @else
            <div style="overflow-x:auto;">
                <table class="fac-table">
                    <thead>
                        <tr><th>Invoice #</th><th>Client</th><th>Type</th><th>Reference</th><th>Amount TTC</th><th>Date</th><th></th></tr>
                    </thead>
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="fac-actions">
                                        <a href="{{ route('admin.factures.show', $facture) }}" class="act-btn view"><i class="fa-solid fa-eye"></i> View</a>
                                        <a href="{{ route('admin.factures.telecharger', $facture) }}" class="act-btn pdf"><i class="fa-solid fa-file-pdf"></i> PDF</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
