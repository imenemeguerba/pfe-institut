<x-app-layout>
<x-slot name="header">Edit Product Category</x-slot>
...
<div class="form-inner">
    <div class="form-top">
        <h1>Edit Category — {{ $categorieProduit->nom }}</h1>
        <a href="{{ route('admin.categories-produits.index') }}" class="btn-back">
            <i class="fa-solid fa-arrow-left"></i> Back
        </a>
    </div>

    <form method="POST" action="{{ route('admin.categories-produits.update', $categorieProduit) }}">
        @csrf @method('PATCH')
        <div class="form-card">
            <div class="form-card-title"><i class="fa-solid fa-folder"></i> Category Information</div>
            <div class="f-group">
                <label class="f-label">Category Name *</label>
                <input type="text" name="nom" value="{{ old('nom', $categorieProduit->nom) }}" required autofocus class="f-input">
                @error('nom')<p class="f-error">{{ $message }}</p>@enderror
            </div>
            <div class="f-group">
                <label class="f-label">Description</label>
                <textarea name="description" class="f-input">{{ old('description', $categorieProduit->description) }}</textarea>
                @error('description')<p class="f-error">{{ $message }}</p>@enderror
            </div>
            <div class="f-group" style="margin-bottom:0;">
                <div class="toggle-row">
                    <div class="toggle-info"><h4>Active Category</h4><p>Visible to clients in the shop</p></div>
                    <label class="toggle-switch">
                        <input type="checkbox" name="actif" value="1" {{ old('actif', $categorieProduit->actif) ? 'checked' : '' }}>
        ...
        <div class="form-footer">
            <a href="{{ route('admin.categories-produits.index') }}" class="btn-cancel">Cancel</a>
