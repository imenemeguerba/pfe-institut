<x-app-layout>
<x-slot name="header">Edit Promo Code</x-slot>
...
<div class="form-inner">
    <div class="form-top">
        <h1>Edit Code — {{ $codePromo->code }}</h1>
        <a href="{{ route('admin.codes-promo.index') }}" class="btn-back"><i class="fa-solid fa-arrow-left"></i> Back</a>
    </div>
    <form method="POST" action="{{ route('admin.codes-promo.update', $codePromo) }}">
        @csrf @method('PUT')
        <div class="form-card">
            <div class="form-card-title"><i class="fa-solid fa-tag"></i> Code & Discount</div>
            <div class="f-group">
                <label class="f-label">Promo Code *</label>
                <input type="text" name="code" id="codeInput" value="{{ old('code', $codePromo->code) }}" required
                       class="f-input f-mono" oninput="this.value=this.value.toUpperCase();updatePreview(this.value)">
                <div class="code-preview" id="codePreview">{{ old('code', $codePromo->code) }}</div>
                @error('code')<p class="f-error">{{ $message }}</p>@enderror
            </div>
            <div class="f-group">
                <label class="f-label">Description (optional)</label>
                <textarea name="description" class="f-input" rows="2" maxlength="500">{{ old('description', $codePromo->description) }}</textarea>
            </div>
            <div class="f-row" style="margin-bottom:0;">
                <div class="f-group" style="margin-bottom:0;">
                    ...
                    <select name="type_reduction" required class="f-input f-select">
                        <option value="pourcentage" {{ old('type_reduction',$codePromo->type_reduction)==='pourcentage'?'selected':'' }}>Percentage (%)</option>
                        <option value="montant"     {{ old('type_reduction',$codePromo->type_reduction)==='montant'?'selected':'' }}>Fixed Amount (DA)</option>
                    </select>
                </div>
                <div class="f-group" style="margin-bottom:0;">
                    <label class="f-label">Value *</label>
                    <input type="number" name="valeur" value="{{ old('valeur',$codePromo->valeur) }}" min="1" required class="f-input">
        ...
        <div class="form-card">
            ...
            <div class="apply-grid">
                <div>
                    <input type="radio" name="applicable_a" id="app_services" value="services" class="apply-option" {{ old('applicable_a',$codePromo->applicable_a)==='services'?'checked':'' }}>
                    <label for="app_services" class="apply-label"><i class="fa-solid fa-spa"></i><span>Services only</span></label>
                </div>
                <div>
                    <input type="radio" name="applicable_a" id="app_produits" value="produits" class="apply-option" {{ old('applicable_a',$codePromo->applicable_a)==='produits'?'checked':'' }}>
                    <label for="app_produits" class="apply-label"><i class="fa-solid fa-box"></i><span>Products only</span></label>
                </div>
                <div>
                    <input type="radio" name="applicable_a" id="app_both" value="les_deux" class="apply-option" {{ old('applicable_a',$codePromo->applicable_a)==='les_deux'?'checked':'' }}>
                    <label for="app_both" class="apply-label"><i class="fa-solid fa-layer-group"></i><span>Both</span></label>
                </div>
            </div>
        </div>
        <div class="form-card">
            ...
            <div class="f-row">
                <div class="f-group" style="margin-bottom:0;">
                    ...
                    <input type="datetime-local" name="date_debut" required class="f-input" value="{{ old('date_debut',$codePromo->date_debut->format('Y-m-d\TH:i')) }}">
                </div>
                <div class="f-group" style="margin-bottom:0;">
                    <label class="f-label">End Date *</label>
                    <input type="datetime-local" name="date_fin" required class="f-input" value="{{ old('date_fin',$codePromo->date_fin->format('Y-m-d\TH:i')) }}">
                </div>
            </div>
            <div class="f-group" style="margin-top:16px;margin-bottom:0;">
                <label class="f-label">Usage Limit (optional)</label>
                <input type="number" name="limite_utilisation" value="{{ old('limite_utilisation',$codePromo->limite_utilisation) }}" min="1" class="f-input" placeholder="Leave empty = unlimited">
                <p class="f-hint">Already used {{ $codePromo->nombre_utilisations }} time(s)</p>
            </div>
        </div>
        <div class="form-card">
            <div class="toggle-row">
                <div class="toggle-info"><h4>Active Code</h4><p>Clients can use this code</p></div>
                <label class="toggle-switch">
                    <input type="checkbox" name="actif" value="1" {{ old('actif',$codePromo->actif)?'checked':'' }}>
...
<script>
function updatePreview(val) { document.getElementById('codePreview').textContent = val || 'MYCODE'; }
