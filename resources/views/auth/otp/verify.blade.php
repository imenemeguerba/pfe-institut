...
.otp-container { width:820px; height:96vh; max-height:560px; background:white; border-radius:30px; box-shadow:0 0 40px rgba(180,128,255,0.2); display:flex; overflow:hidden; }
...
.email-badge { display:block; text-align:center; background:rgba(180,128,255,0.1); color:#b480ff; padding:5px 14px; border-radius:20px; font-size:13px; font-weight:600; margin-bottom:24px; }
.otp-input-wrap { display:flex; justify-content:center; gap:10px; margin:16px 0; }
.otp-input-wrap input { width:52px; height:60px; text-align:center; font-size:24px; font-weight:700; background:#f0f0f0; border:2px solid transparent; border-radius:12px; outline:none; color:#333; font-family:'Segoe UI',sans-serif; transition:all 0.2s; }
...
.resend-btn { background:transparent; border:1.5px solid #b480ff; cursor:pointer; font-size:13px; color:#b480ff; font-family:'Segoe UI',sans-serif; padding:7px 20px; border-radius:30px; transition:0.3s; }
...
<div class="otp-container">
    ...
    <div class="otp-right">
        ...
        <span class="email-badge">{{ $email ?? session('otp_email') ?? '' }}</span>
        ...
        <form method="POST" action="{{ route('password.otp.verify.post') }}" id="otpForm">
            @csrf
            {{-- ✅ FIX: email requis par le controller --}}
...
digits.forEach(function(input, i) {
    ...
    input.addEventListener('paste', function(e) {
        ...
        Array.from(paste).forEach(function(char, j) { if (digits[i+j]) digits[i+j].value = char; });
        updateHidden();
        var target = digits[Math.min(i+paste.length, digits.length-1)];
        if (target) target.focus();
    });
});
function updateHidden() {
    document.getElementById('otpHidden').value = Array.from(digits).map(function(d){ return d.value; }).join('');
}
