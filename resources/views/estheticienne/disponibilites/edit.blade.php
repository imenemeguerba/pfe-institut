<x-app-layout>
<x-slot name="header">Edit Availability Slot</x-slot>
...
<div class="cr-hero">
    ...
    <div class="cr-hero-content">
        <div>
            <div class="cr-hero-tag"><i class="fa-solid fa-pen"></i> Edit Slot</div>
            <div class="cr-hero-title">Edit <span>Availability</span></div>
        </div>
...
<div class="form-body"><div class="form-inner">
    <form method="POST" action="{{ route('estheticienne.disponibilites.update', $disponibilite) }}">
        @csrf @method('PATCH')
        <div class="form-card">
            ...
            <div class="f-group">
                ...
                <select name="jour_semaine" required class="f-input f-select">
                    @foreach([1=>'Monday',2=>'Tuesday',3=>'Wednesday',4=>'Thursday',5=>'Friday',6=>'Saturday',7=>'Sunday'] as $val=>$lbl)
                        <option value="{{ $val }}" {{ old('jour_semaine',$disponibilite->jour_semaine)==$val?'selected':'' }}>{{ $lbl }}</option>
                    @endforeach
                </select>
            ...
            <div class="presets-grid">
                <button type="button" class="preset-btn morning" onclick="setTime('08:00','12:00')">☀️ Morning</button>
                <button type="button" class="preset-btn afternoon" onclick="setTime('13:00','19:00')">🌙 Afternoon</button>
                <button type="button" class="preset-btn fullday" onclick="setTime('08:00','19:00')">📅 Full Day</button>
            </div>
            <div class="f-row">
                <div class="f-group" style="margin-bottom:0;">
                    <label class="f-label">Start Time *</label>
                    <input type="time" id="heure_debut" name="heure_debut" required class="f-input"
                           value="{{ old('heure_debut',\Carbon\Carbon::parse($disponibilite->heure_debut)->format('H:i')) }}">
                    @error('heure_debut')<p class="f-error">{{ $message }}</p>@enderror
                </div>
                <div class="f-group" style="margin-bottom:0;">
                    <label class="f-label">End Time *</label>
                    <input type="time" id="heure_fin" name="heure_fin" required class="f-input"
                           value="{{ old('heure_fin',\Carbon\Carbon::parse($disponibilite->heure_fin)->format('H:i')) }}">
